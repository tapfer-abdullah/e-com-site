    ...
    <div className="px-5 pb-5 w-full">
      <div className="mb-5 flex justify-between items-center px-3 py-2 bg-[#d5ddda] shadow-lg rounded-lg">
        <h1 className="text-xl font-semibold">
          All Discount Codes <span>({discountCodes?.length})</span>
        </h1>
        <div className="flex items-center gap-3 px-2">
          ...
          >
            <option disabled value="FilterDiscount">
              ...
              {discountCodes?.map((d) => (
                <tr key={d?._id}>
                  <td className=" font-semibold">
                    <Link href={`/dashboard/discount/${d?._id}`}>{d?.title}</Link>
                  </td>
                  ...
                  <td className="capitalize !text-left">
                    {d?.discountCodeType == "BxGy" && (
                      <>
                        {d?.additionalData?.BxGy?.BxGyType} {d?.additionalData?.BxGy?.CusBuyAmount}
                        <> get </>
                        {d?.additionalData?.BxGy?.DiscountedType?.option == "free" ? (
                          <span>
                            {d?.additionalData?.BxGy?.CusGetAmount} {d?.additionalData?.BxGy?.DiscountedType?.option}
                          </span>
                        ) : (
                          <span>
                            {d?.additionalData?.BxGy?.DiscountedType?.value} {d?.additionalData?.BxGy?.DiscountedType?.option == "percentage" ? "%" : "amount"} off of{" "}
                            {d?.additionalData?.BxGy?.CusGetAmount} product
                          </span>
                        )}
                      </>
                    )}

                    {d?.discountCodeType == "AOffP" && (
                      <>
                        {d?.additionalData?.AOffP?.DiscountedType?.value} {d?.additionalData?.AOffP?.DiscountedType?.option == "Fixed" ? "amount" : "%"} off ( on specific{" "}
                        {d?.additionalData?.AOffP?.ApplyTo?.option} )
                      </>
                    )}
                    {d?.discountCodeType == "AOffO" && (
                      <>
                        {d?.additionalData?.AOffO?.DiscountedType?.value} {d?.additionalData?.AOffO?.DiscountedType?.option == "Fixed" ? "amount" : "%"} off on order
                      </>
                    )}
                  ...
                  <td className="!text-center">
                    <div className="!w-full !h-full flex items-center justify-center gap-2">
                      <MdDelete
                        ...
                        className="text-3xl font-bold bg-red-600 text-white rounded-md cursor-pointer p-1"
                      />
                      <Link href={`/dashboard/discount/${d?._id}`}>
                        <MdEditSquare className="text-3xl font-bold bg-[#FFC520] text-white rounded-md cursor-pointer p-1" />
                      </Link>
                    </div>
                  </td>
                </tr>
