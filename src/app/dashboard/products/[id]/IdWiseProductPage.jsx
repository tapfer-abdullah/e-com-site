        ...
        <div className="my-5 mb-16">
          <div className="w-4/5 mx-auto mb-5 flex justify-between items-center px-3 py-2 bg-[#d5ddda] shadow-lg rounded-lg">
            <div className="flex items-center gap-2">
              <Link href={"/dashboard/products"} className="text-xl font-semibold">
                All Products
              </Link>
              <IoIosArrowForward />
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/Collections/${singleProductData?.category?.[0]?.label.toLowerCase()}/${singleProductData?._id}`}
                target="_blank"
                className="py-2 px-4 font-semibold bg-white hover:bg-opacity-80 flex items-center gap-1 border-2 rounded-lg transition-all duration-300"
              >
              ...
              <button
                onClick={() => {
                  handleDelete(singleProductData?._id);
                }}
          ...
          <div className="space-y-3 w-4/5 mx-auto bg-white p-5 rounded-xl shadow-xl">
            ...
            <div className="flex items-center gap-2">
              <div className="w-full space-y-1 md:max-w-full lg:max-w-md">
                ...
                <Select
                  value={selectedType}
                  onChange={(selectedOptions) => setSelectedType(selectedOptions)}
                  required
                  options={types}
                  isMulti={false}
                  isClearable={false}
            ...
            <div className="card2 max-h-[400px] overflow-y-auto">
              {imageURL.length != 0 ? (
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="bg-[#0086fe] text-white py-1 px-3 rounded-lg font-semibold"
                  >
                    + variant
                  </button>
                </div>
              ...
              {isVariant && (
                <div className="relative w-full flex items-center justify-center gap-2 my-3">
                  <input placeholder="Enter image url.." onChange={(e) => setVariantIMG(e.target.value)} type="text" name="variantImg" id="" className="border w-full pl-8 pr-2 py-2 outline-1" />
                  <button
                    onClick={() => {
                      setVariant(false);
                    }}
                    className="bg-red-600 text-white py-1 px-3 rounded-lg font-semibold"
                  >
                    Cancel
                  </button>
                  <button onClick={handleNewVariant} className="bg-[#FFC520] py-1 px-3 rounded-lg font-semibold">
                    Add
                  </button>
              ...
              {imageURL?.map((i, index) => (
                <div key={index} className="grid grid-cols-4 gap-2 items-start justify-center my-3 bg-[#f4f3f9] p-2 rounded-md">
                  <div className="relative flex flex-col gap-2 justify-center items-center">
                    ...
                    <CreatableSelect
                      defaultValue={{ label: i?.name, value: i?.label }}
                      isMulti={false}
                      onChange={(selectedOptions) => handleColor(index, i, selectedOptions)}
                      required
                      options={colors}
                    ...
                    <RiImageEditLine
                      title="Change photo"
                      onClick={() => {
                        setChangeVariantImgBtn(true);
                      }}
                      className="cursor-pointer absolute top-1 right-2 text-2xl p-1 bg-[#FFC520] hover:bg-opacity-80 rounded-full"
                    />

                    {isChangeVariantImgBtn && (
                      <div className="!w-full !h-full rounded-md bg-black bg-opacity-70 absolute top-0 left-0 pt-14 px-1">
                        <input
                          onChange={(e) => setChangeVariantIMG(e.target.value)}
                          placeholder="Enter url"
                          type="text"
                          name="variantImg"
                          id=""
                          className="border-2 rounded-lg w-full pl-8 pr-2 py-2 outline-1"
                        />
                        <div className="w-full flex items-center justify-center gap-2 my-3">
                          ...
                          <button onClick={() => handleVariantIMGChange(index)} className="bg-[#FFC520] py-1 px-3 rounded-lg font-semibold">
                            Add
                          </button>
                  ...
                  <div className="relative col-span-3 start-1 w-full grid grid-cols-3 gap-2 justify-center items-start my-3">
                    {selectedSizes?.map((ss, indexS) => (
                      <div key={indexS} className="w-full">
                        <TextField
                          defaultValue={i?.allSKU?.[indexS]?.sku}
                          onBlur={(e) => handleSKU(e.target.value, indexS, index)}
                          type="text"
                          name="color-wise-sku"
                          id={ss?.value}
                          className="rounded-md w-full border-2 my-2"
                          label={`SKU for ${ss?.value}`}
                        />
                      </div>
                    ))}
                    <RxCross2
            ...
            <div className="w-full flex items-center gap-2">
              <button
                disabled={title.length == 0}
                onClick={handleDiscard}
                className={`w-full flex gap-2 justify-center items-center bg-[#FFC520] ${title.length == 0 ? "opacity-50" : "hover:bg-opacity-80"} py-2 px-4 rounded-lg font-semibold cursor-pointer`}
              >
                Discard
              </button>
              <button
                disabled={title.length == 0}
                onClick={handleUpdateProduct}
                className={`w-full flex gap-2 justify-center items-center bg-green-500 ${title.length == 0 ? "opacity-50" : "hover:bg-opacity-80"} py-2 px-4 rounded-lg font-semibold cursor-pointer`}
              >
                Save
              </button>
            </div>
...
export default IdWiseProductPage;
