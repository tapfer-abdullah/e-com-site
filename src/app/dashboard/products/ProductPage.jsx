              ...
              <tr key={p?._id}>
                <td>
                  <Link href={`/dashboard/products/${p?._id}`}>
                    <img src={p?.imageUrl?.[0]} alt="img" className="w-12 h-12" />
                  </Link>
                </td>
                <td>
                  <Link href={`/dashboard/products/${p?._id}`}>{p?.title?.length > 20 ? <>{p?.title?.slice(0, 20)}..</> : <>{p?.title?.slice(0, 10)}</>}</Link>
                </td>
                <td>{p?.price}</td>
                ...
                <td className="!text-center">
                  <div className="!w-full !h-full flex items-center justify-center gap-2">
                    ...
                    <Link target="_blank" href={`/Collections/${p?.category?.[0]?.label.toLowerCase()}/${p?._id}`}>
                      <FaArrowUpRightFromSquare className="text-3xl font-bold bg-green-500 text-white rounded-md cursor-pointer p-1" />
                    </Link>
                  </div>
                </td>
              </tr>
